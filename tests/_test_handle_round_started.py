...
class TestRoundStarted:
    ...
    def test_round_started_returns_list_with_5_elements(self, default_state):
        events = handle_round_started(default_state, None, 0)
        assert len(events) == 5


    def test_round_started_return_list_elements_are_instance_of_event(self, default_state):
        events = handle_round_started(default_state, None, 0)
    ...
    def test_round_started_return_list_in_with_correct_target_and_in_correct_order(self, default_state):
        events = handle_round_started(default_state, None, 0)
